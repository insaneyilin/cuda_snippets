...
__global__ void Kernel(Sphere *spheres, unsigned char *ptr) {
...
int main(int argc, char **argv) {
  ...
  CHECK_CUDA_ERROR(cudaMalloc((void**)&dev_bitmap,
      image_data_size));
  // allocate memory on the GPU for the Sphere dataset
  CHECK_CUDA_ERROR(cudaMalloc((void**)&spheres,
  ...
  CHECK_CUDA_ERROR(cudaMemcpy(spheres, temp_spheres,
                              sizeof(Sphere) * NUM_SPHERES,
  ...
  Kernel<<<grid_size, block_size>>>(spheres, dev_bitmap);
  ...
  CHECK_CUDA_ERROR(cudaFree(spheres));

  image.Write("ray_tracing_no_const_mem.png");
